...
try:
    ...
    for f in FILES:
        if f == "cvt.ini" and PACKAGE_VERSION:  # Replace {version} in cvt.ini
            with open(Path.joinpath(Path('..'), f), "r", encoding="utf-8") as cvtinif:
                cvtini = ''.join(cvtinif.readlines())
                cvtini = cvtini.replace("{version}", PACKAGE_VERSION)
                zf.writestr(f, cvtini.encode())
            continue
