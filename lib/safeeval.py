...
# -*- coding: utf-8 -*-
#
# Added: support for full floating point format, fixed evaluating exp like 60*60 by adding float()s
#
# Based on Axiacore/py-expression-eval, by AxiaCore S.A.S. http://axiacore.com
#
...
class Parser:
    ...
    def __init__(self):
        ...
        self.re_number = re.compile(r'(?P<number>[-+]?[0-9]+(?:\.?[0-9]+)?(?:[eE][-+]?[0-9]+)?)')
    ...
    def isNumber(self):
        parsed_num = self.re_number.match(self.expression[self.pos:])
        if parsed_num is None:
            return False

        self.tokennumber = parsed_num["number"]
        self.pos += len(self.tokennumber)
        return True
    ...
    def isVar(self):
        ...
        for i in range(self.pos, len(self.expression)):
            c = self.expression[i]
            if c.lower() == c.upper():
                if ((i == self.pos and c != '"') or
                   (c not in '_."' and (c < '0' or c > '9'))) and not inQuotes:
                    break
            if c == '"':
